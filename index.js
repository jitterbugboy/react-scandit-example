...
export default class App extends Component {
  ...
  onButtonClick() {
    ...
    BarcodePicker.create(document.getElementById('scandit-barcode-picker'), {
      ...
      barcodePicker.onReady(() => {
        const timeElapsed = (performance.now() - self.state.openClickTs) / 1000;
        self.setState({timeToInit: timeElapsed.toFixed(2) + 's'});
  ...
  render() {
    ...
    const message = this.state.timeToInit;
    return (
      <div>
        <p>
          This takes very long before scanner is ready
          {message}
        </p>
